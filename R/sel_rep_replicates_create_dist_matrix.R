...
##' @export
sel_rep_replicates_create_dist_matrix <- function(subject_name,sel_normal_sample,all_normal_samples,pdir='.',overwrite=F,sample_exclusion_th=0.15,max_replicate_th=0.11,intfraction_cutoff=0.1,abovebelow_cutoff=0.15,distcut=0.06,file_filter=0.1,rob_th=0.05) {
    ...
    for (markers in marker_list) {
        ...
        distance_distribution_heatmap(filepath=ddh_fp,subject_name,markers,max_replicate_th,sel_normal_sample,all_normal_samples,rob_th,subject_output_list)
    }

    ## uncomment this!
    #if (subject_name_change) {
    #    source("polygR/distance_distribution_heatmap_sampleNameChange.R")
    #} else {
    #    source("polygR/distance_distribution_heatmap.R")
    #}
    #
