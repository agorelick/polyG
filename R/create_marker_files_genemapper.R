##' create_marker_files_genemapper
##' @export
create_marker_files_genemapper <- function(sample_table_file, peak_table_file, markers_file, mixes_file, sizing_artifacts_file=NA, pdir='.', hcut=0.015, thres=20, intensityfilter=0.1) {
    ...
    info <- read.table(markers_file,sep="\t",header=T,stringsAsFactors=F)
    mixes <- read.table(mixes_file,sep="\t",header=F,stringsAsFactors=F)

    # remove samples with failed sizing
    failedsize <- samp$Sample.Name[which(samp$SQ==0)]
    if (length(which(samp$SQ==0))!=0) samp <- samp[-which(samp$SQ==0),]

    # sizing artifacts file
    if (!is.na(sizing_artifacts_file)) sizearti <- read.table(sizing_artifacts_file,sep="\t",header=F,stringsAsFactors=F)
    ...
    rec.samples <- for (i in 2:dim(peak)[1]){
        if (peak$Sample.File.Name[i]==t) next
        else {
            end <- c(end,i-1)
            begin <- c(begin,i)
    ...
    for (k in 1:nmarkers){
        ...
        for (j in 2:dim(info)[2]){
            ...
            #if (set==0) finalnlist[[j-1]] <- finalnames[grep(patient,finalnames)]
            ...
            write.table(heightmat,file.path(data_dir,paste(marker,"_",patient,"_",intensityfilter,".txt",sep="")),sep="\t",quote=F,row.names=F,col.names=as.data.frame(strsplit(colnames(heightmat),"\\."),stringsAsFactors=F)[2,])
